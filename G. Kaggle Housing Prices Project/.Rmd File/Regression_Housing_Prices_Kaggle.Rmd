...
**Note: This is not my original work. I followed this [guide](https://www.kaggle.com/erikbruin/house-prices-lasso-xgboost-and-a-detailed-eda/report).** 
...
Displaying the first 10 variables and the response variable.
```{r}
str(train[,c(1:10, 81)])
```
...
Without the `Id`, the data frames combined consists of 79 predictors and our response variable `SalePrice`.
```{r}
test$SalePrice <- NA
...
Making a histogram of the response variable `SalePrice`.
```{r}
...
Examining a summary of `SalePrice`.
```{r}
...
Creating indices and names for the numeric variables.
```{r}
...
Obtaining correlations for all of the numeric variables and plotting. 
```{r}
all_numVar <- all[, numericVars] #selecting only the numeric variables
cor_numVar <- cor(all_numVar, use="pairwise.complete.obs") #correlations of all numeric variables in the form of a matrix

#sort on decreasing correlations with SalePrice
cor_sorted <- as.matrix(sort(cor_numVar[,'SalePrice'], decreasing = TRUE))

#select only high correlations and variable names 
CorHigh <- names(which(apply(cor_sorted, 1, function(x) abs(x)>0.5)))

#updating the correlation matrix to show only variables with high correlations with the outcome 
...
In the remainder of this section, I will visualize the relation between SalePrice and the two predictors with the highest correlation with `SalePrice`; `OverallQual` and the `GrLivArea` (this is the proportion of the house that is not in a basement).

It also becomes clear that multicollinearity is an issue. For example: the correlation between `GarageCars` and `GarageArea` is very high (0.89), and both have similar (high) correlations with `SalePrice`. Multicollinearity is also present in other variables, such as `X1stFlrSF` and `TotalBsmtS`. Both have 0.61 correlations with the outcome and a correlation of 0.80 with each other. The other 6 variables with a correlation higher than 0.5 with `SalePrice` are:
...
`OverallQual` has the highest correlation with `SalePrice` among the numeric variables (0.79). It rates the overall material and finish of the house on a scale from 1 (very poor) to 10 (very excellent).
```{r}
...
The numeric variable with the second highest correlation with `SalesPrice` is the `GrLivArea`. This make a lot of sense; big houses are generally more expensive.
```{r message=FALSE, warning=FALSE}
ggplot(data=all[!is.na(all$SalePrice),], aes(x=GrLivArea, y=SalePrice))+
        geom_point(col='blue') + geom_smooth(method = "lm", se=FALSE, color="black", aes(group=1)) +
        scale_y_continuous(breaks= seq(0, 800000, by=100000), labels = comma) +
        geom_text_repel(aes(label = ifelse(all$GrLivArea[!is.na(all$SalePrice)]>4500, rownames(all), ''))) #this last line is subsetting all$GrLivArea and selecting only observations that are not missing on the outcome
```

Especially the two houses with really big living areas and low `SalesPrice` seem like outliers (houses 524 and 1299, see labels in graph). I will not take them out yet, as taking outliers can be dangerous. For instance, a low score on the `OverallQual` could explain a low price. However, as you can see below, these two houses actually also score maximum points on `OverallQual`. Therefore, I will keep houses 1299 and 524 in mind as prime candidates to take out as outliers.
...
**Note: There are more advanced and less time-intensive methods to handling missing data. This was only done to replicate the results of the author.**
...
Now, we will impute the missing data.
...
The `PoolQC` is the variable with most NAs. The description of the data says that NA means there are no pools. 
```{r eval=FALSE}
   Ex   Excellent
   Gd   Good
   TA   Average/Typical
...
Within `MisceFeature`, there are 2814 NAs. The description of the data shows that NA for `MiscFeature` means None. 
```{r eval=FALSE}
   Elev Elevator
...
As the values are not ordinal, I will convert `MiscFeature` into a factor.
...
Within Alley, there are 2721 NAs. As the values are not ordinal, I will convert Alley into a factor. The NA values mean that there is no alley access.
```{r eval=FALSE}
   Grvl Gravel
   Pave Paved
   NA   No alley access
...
Within `Fence`, there are 2348 NAs. The values seem to be ordinal. The description of the data shows that NA means there is no fence. 
```{r eval=FALSE} 
   GdPrv    Good Privacy
...
Within `FireplaceQu`, there are 1420 NAs. `Fireplaces` is complete.
...
There are 3 lot variables. One with NAs, and 2 complete variables.

For `LotFrontage`, there are 486 NAs. The most reasonable imputation seems to take the median per neighborhood.
```{r}
...
Imputing `LotFrontage` with median per neighborhood.
```{r}
for(i in 1:nrow(all)){
...
For `LotShape` there are no NAs. Values seem ordinal (Regular=best).
```{r eval=FALSE}
   Reg  Regular 
...
`LotConfig` does not have any missing values. The values seemed possibly ordinal to me, but the visualization does not show this. Therefore, I will convert the variable into a factor.
```{r eval=FALSE}
   Inside   Inside lot
   Corner   Corner lot
   CulDSac  Cul-de-sac
...
Two of those have one NA (`GarageCars` and `GarageArea`), one has 157 NAs (`GarageType`), and 4 variables have 159 NAs.
...
Now, to find the 2 additional NAs.
```{r}
...
`GarageCars` and `GarageArea` both have 1 NA. As you can see above, it is house 2577 for both variables. The problem probably occurred as the `GarageType` for this house is “detached”, while all other Garage-variables seem to indicate that this house has no Garage.
```{r}
...
For `GarageType`, the values do not seem ordinal, so I will convert into a factor.
```{r eval=FALSE}
...
For `GarageFinish`, the values are ordinal.
```{r eval=FALSE}
   Fin  Finished
   RFn  Rough Finished  
   Unf  Unfinished
...
`GarageQual` is another variable than can be made ordinal with the `Qualities` vector.
```{r eval=FALSE}
   Ex   Excellent
   Gd   Good
   TA   Typical/Average
...
`GarageCond` is another variable than can be made ordinal with the `Qualities` vector.
```{r eval=FALSE}
   Ex   Excellent
   Gd   Good
   TA   Typical/Average
...
```{r}
#Find the additional NAs; BsmtFinType1 is the one with 79 NAs, so we need to find out where the NAs in the other basement variables come from 
...
`BsmtQual` is a variable than can be made ordinal with the `Qualities` vector.
```{r eval=FALSE}
...
`BsmtCond` is a variable than can be made ordinal with the `Qualities` vector.
```{r eval=FALSE}
   Ex   Excellent
   Gd   Good
...
`BsmtExposure` is a variable than can be made ordinal.
```{r eval=FALSE}
   Gd   Good Exposure
...
`BsmtFinTyp1` is a variable than can be made ordinal.
```{r eval=FALSE}
...
`BsmtFinTyp2` is a variable than can be made ordinal with the `FinType` vector.
```{r eval=FALSE}
   GLQ  Good Living Quarters
   ALQ  Average Living Quarters
   BLQ  Below Average Living Quarters   
...
`BsmtFullBath` is an integer variable.
```{r}
all$BsmtFullBath[is.na(all$BsmtFullBath)] <- 0
table(all$BsmtFullBath)
```

`BsmtHalfBath` is an integer variable.
```{r}
all$BsmtHalfBath[is.na(all$BsmtHalfBath)] <- 0
table(all$BsmtHalfBath)
```

`BsmtFinSF1` is an integer variable.
```{r}
all$BsmtFinSF1[is.na(all$BsmtFinSF1)] <- 0
```

`BsmtFinSF2` is an integer variable.
```{r}
all$BsmtFinSF2[is.na(all$BsmtFinSF2)] <-0
```

`BsmtUnfSF` is an integer variable.
```{r}
all$BsmtUnfSF[is.na(all$BsmtUnfSF)] <- 0
```

`TotalBsmtSF` is an integer variable.
```{r}
...
We will check the ordinality of `MasVnrType` below.
```{r eval=FALSE}
   BrkCmn   Brick Common
   BrkFace  Brick Face
   CBlock   Cinder Block
...
`MasVnrArea` is an integer variable.
```{r}
...
`MSZoning` has 4 NAs. The values are categorical.
```{r eval=FALSE}
   A    Agriculture
   C    Commercial
...
Kitchen quality has 1NA. Can be made ordinal with the `Qualities` vector.
```{r eval=FALSE}
   Ex   Excellent
   Gd   Good
   TA   Typical/Average
...
`KitchenAbvGr` is an integer variable with no NAs.
```{r}
...
```{r}
all[is.na(all$Utilities) | all$Utilities=='NoSeWa', 1:9]
```
...
1 NA. Can be made ordinal (salvage only is worst, typical is best).
```{r eval=FALSE}
...
`Exterior1st` has 1 NA. Values are categorical.
```{r eval=FALSE}
...
`Exterior2nd` has 1 NA. Values are categorical.
```{r eval=FALSE}
   AsbShng  Asbestos Shingles
   AsphShn  Asphalt Shingles
   BrkComm  Brick Common
...
`ExterQual` has no NAs. Can be made ordinal using the `Qualities` vector.
```{r eval=FALSE}
   Ex   Excellent
   Gd   Good
   TA   Average/Typical
...
`ExterCond` has no NAs. Can be made ordinal using the `Qualities` vector.
```{r eval=FALSE}
   Ex   Excellent
   Gd   Good
   TA   Average/Typical
...
`SaleType` has 1 NA. Values are categorical.
```{r eval=FALSE}
...
`SaleCondition` has no NAs. Values are categorical.
```{r eval=FALSE}
   Normal   Normal Sale
...
At this point, I have made sure that all variables with NAs are taken care of. However, I still need to also take care of the remaining character variables without missing values. Similar to the previous section, I have created tabs for groups of variables.
```{r}
...
No ordinality, so it will be converted to a factor.
```{r}
...
No ordinality, so converting to a factor.
```{r}
...
This includes 2 variables that specify the flatness and slope of the property.

The first is `LandContour`:
```{r eval=FALSE}
   Lvl  Near Flat/Level 
...
It is ordinal, so label encoding
```{r}
...
There are 2 variables that specify the type and style of dwelling.

The first is `BldgType`.
```{r eval=FALSE}
...
However, the visualization does not show ordinality, so it will be converted to a factor.
```{r}
...
No ordinality, so will be converted to factor.
```{r}
...
There are 3 variables that specify the physical location, and the proximity of ‘conditions’.

The first is `Neighborhood`:
```{r eval=FALSE}
...
There is no ordinality, so converting to factor.
```{r}
...
No ordinality, so converting into a factor. 
```{r}
...
This is ordinal, so doing label encoding.
```{r}
...
The first is year and month sold.
...
Although possible, it is a bit less steep than expected, the effects of the Banking crises that took place at the end of 2007 can be seen indeed. After the highest median prices in 2007, the prices gradually decreased. However, seasonality seems to play a bigger role, as you can see below.
```{r}
...
Although the correlations are giving a good overview of the most important numeric variables and multicollinerity among those variables, I wanted to get an overview of the most important variables including the categorical variables before moving on to visualization.
...
#index 79 is the outcome variable 
...
Only 3 of those most important variables are categorical according to the random forest; `Neighborhood`, `MSSubClass`, and `GarageType`.

Now we will look at Above Ground Living Area, and other surface related variables (in square feet). As I have already visualized the relationship between the Above Ground Living Area and `SalePrice` in my initial explorations, I will now just display the distribution itself. As there are more ‘square feet’ surface measurements in the top 20 of the quick random forest, I am taking the opportunity to bundle them in this section. Note: `GarageArea` is taken care of in the Garage variables section.
...
Now to examine the most important categorical variable: `Neighborhood`.
...
The second most important categorical variable is `MSSubClass`. The first visualization shows the median `SalePrice` by `MSSubClass`. The frequency (number of houses) of each `MSSubClass` in the train set is shown in the labels. 

The second graph shows the frequencies across all data. Most houses are relatively new, and have one or two stories.
```{r}
...
Several Garage variables have a high correlation with `SalePrice`, and are also in the top 20 list of the quick random forest. However, there is multicollinearity among them and I think that 7 garage variables is too many anyway. I feel that something like 3 variables should be sufficient (possibly `GarageCars`, `GarageType`, and a `GarageQual` measurement), but before I do any selection I am visualizing all of them in this section.
```{r}
#correct error
...
Both the median and mean Saleprices agree on 3 neighborhoods with substantially higher sales prices. The separation of the 3 relatively poor neighborhoods is less clear, but at least both graphs agree on the same 3 poor neighborhoods. Since I do not want to ‘overbin’, I am only creating categories for those ‘extremes’.
```{r}
...
Now, we will consolidate the porch variables. 
...
Now, moving onto removing outliers.

For the time being, I am keeping it simple and just remove the two really big houses with low `SalePrice` manually. However, I intend to investigate this more thoroughly in a later stage (possibly using the ‘outliers’ package).
```{r}
all <- all[-c(524, 1299),]
```

Before modeling I need to center and scale the ‘true numeric’ predictors (so not variables that have been label encoded), and create dummy variables for the categorical predictors. Below, I am splitting the data frame into one with all (true) numeric variables, and another data frame holding the factors.
```{r}
#we are going to select all the numeric variables, except for a few - this is because they have either been combined, transformed, etc.
...
The last step needed to ensure that all predictors are converted into numeric columns (which is required by most Machine Learning algorithms) is to ‘one-hot encode’ the categorical variables - AKA create dummy variables. This basically means that all (not ordinal) factor values are getting a separate columns with 1s and 0s (1 basically means Yes/Present). To do this one-hot encoding, I am using the `model.matrix()` function.
```{r}
DFdummies <- as.data.frame(model.matrix(~.-1, DFfactors))
dim(DFdummies)
```

In previous versions, I worked with Caret’s Near Zero Variance function. Although this works, it also is a quick fix and too much information got lost. For instance, by using the defaults, all Neighborhoods with less than 146 houses are omitted as (one-hot encoded) variables (frequency ratio higher than 95/5). Therefore, I have taken a more carefull manual approach in this version to remove levels wit hfew or no observations in train or test.
```{r}
...
```{r}
combined <- cbind(DFnorm, DFdummies) #combining all (now numeric) predictors into one data frame
```

Now to deal with the skewness in the response variable `SalePrice`.
```{r}
skew(all$SalePrice)
...
Now, to compose the train and test sets.
```{r}
...
Initially, I just worked with the `XGBoost` package directly. The main reason for this was that the package uses its own efficient data structure (`xgb.DMatrix`). The package also provides a cross validation function. However, this CV function only determines the optimal number of rounds, and does not support a full grid search of hyperparameters.

Although `caret` does not seem to use the (fast) data structure of the `xgb` package, I eventually decided to do hyperparameter tuning with it anyway, as it at least supports a full grid search. As far as I understand it, the main parameters to tune to avoid overfitting are `max_depth`, and `min_child_weight` (see `XGBoost` documentation). Below I am setting up a grid that tunes both these parameters, and also the eta (learning rate).
```{r}
...
The next step is to let caret find the best hyperparameter values (using 5 fold cross validation). It will not be run because it takes very long.
```{r eval=FALSE}
...
As expected, this took quite a bit of time (locally). According to `caret`, the ‘bestTune’ parameters are:
...
Now, to see the variable importance plot.
```{r message=FALSE, warning=FALSE}
...
Now, we will average the predictions. Since the lasso and XGBoost algorithms are very different, averaging predictions likely improves the scores. As the lasso model does better regarding the cross validated RMSE score (0.1121 versus 0.1162), I am weighting the lasso model double.
```{r}
