...
Prior to coding the algorithm, let's create a dataset that will be used to test it. For this, we will modify the built-in `iris` dataset. For the outcome, we will only be using `setosa` and `versicolor`. To keep it simple, from $x$ variables, we will only be using `Sepal.Length` and `Petal.Length`.
```{r}
